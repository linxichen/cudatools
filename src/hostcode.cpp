...
// 				P = Vectorized transition matrix (N^M by N^M)
	...
	Ssigma_e.print("Var-Cov Matrix of Innovations:");
...
double levelOLS(double* Y_ptr, double** X_ptrptr, int nobs, int nrhs, double* bbeta_ptr) {
	// concatenate X matrix
	mat X(nobs,nrhs);
...
double logOLS(double* Y_ptr, double** X_ptrptr, int nobs, int nrhs, double* bbeta_ptr) {
	// concatenate X matrix
	mat X(nobs,nrhs);
	for (int j = 0; j < nrhs; j++) {
		for (int i = 0; i < nobs; i++) {
